on *:LOAD: { 
  .echo -s 0,0 -
  .echo -s 0,4#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
  .echo -s 4[ 94mircX.org 1© 4] 4S94ecure 4Q94uery 4S94ystem 94| 4ACTIVATED 94|
  .echo -s 0,4#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
  .echo -s 0,0 -
}
...
on *:UNLOAD: { 
  .echo -s 0,0 -
  .echo -s 0,4#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
  .echo -s 4[ 91mircX.org 1© 4] 4S94ecure 4Q94uery 4S94ystem 94| 4DE-ACTIVATED 94|
  .echo -s 0,4#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#
  .echo -s 0,0 -
}
alias _vr {
...
menu query,menubar {
  ...
  .ON $sqo: _vw querycon Activate (ON)
  .OFF $sqf: _vw querycon Deactivate (OFF)
}
menu status {
  Secure Query System
  .Config:secure.query
...
dialog querycon {
  ...
  box "Audible Alert On Private Messages", 17, 8 75 80 22, tab 2
